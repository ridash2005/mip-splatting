\chapter{Masking the view-dependent colour a capture cannot determine}
\label{ch:method2}
...
determine, but they share neither a mechanism nor a failure mode. The angular mask also carries a memory result that the anisotropic filter does
not, since masked coefficients need not be stored.}

\section{Making the threshold $\tau$ mean the same thing on every capture}
\label{sec:b2scale}
