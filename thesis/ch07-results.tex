...
codebase, and that G2 was therefore unmeetable here as a property of the gate's premise.
...
And it trains without incident on arm A, so the fragility belongs to the vanilla band-limit semantics.
...
of \emph{every} scene. By Proposition~\ref{prop:reduction} that makes B1 \emph{be} Mip-Splatting on this benchmark, exactly, which is worth stating plainly:
...
then makes B1 and Mip-Splatting \emph{identical}. So
...
any direction, $\Sigma_{\mathrm{filt}} = f_k^2 I$ identically and B1 \emph{is} Mip-Splatting: the same filter, taking the same code path.
