...
implementation and one LPIPS backbone (VGG), so any difference between them is the method and not the plumbing. The diff that separates the arms is
...
\begin{figure}[htbp]
  ...
  that half the images are downsampled (drawn hollow), which is a defect of camera \emph{settings} and not of geometry; and \emph{grazing} keeps a wide
...
capture the prediction was made for, and the prediction must then be re-evaluated at the span that was built, not the one that was asked for.
