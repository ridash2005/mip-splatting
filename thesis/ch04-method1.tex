\chapter{An anisotropic band-limit derived from the capture's conditioning}
\label{ch:method1}
...
\section{Treating primitive position as a quantity the photographs measure}
...
\section{The measurement Jacobian the renderer already forms}
\label{sec:jacobian}
...
\section{Accumulating over views: the observability matrix $\Lambda_k$}
...
\section{From accumulated information to a covariance floor}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  \item $\sigma_{\mathrm{pix}} = \sqrt{0.2} = 0.447$\,px is \emph{inherited} from Mip-
  Splatting's own constant. Proposition~\ref{prop:constant} shows it is the localisation
...
\section{A single view is already anisotropic, falling off as $\cos^2\varphi$}
\label{sec:offaxis}
...
\section{Two views: the exact triaxial spectrum, and where it inverts}
...
Notice that these are three distinct numbers. The right filter is therefore triaxial, with a different limit along each of
...
\subsection{A capture is a distribution of views, and two views understate it by $\sqrt{2}$}
...
\section{Exact reduction: why the method equals Mip-Splatting on a good capture}
...
\S\ref{sec:onecodebase}, and Chapter~\ref{ch:results} reports it verified on the saved models.

\section{What a matrix filter costs the implementation}
\label{sec:implcost}
...
\section{Arithmetic cost per construction, and how often it is paid}
