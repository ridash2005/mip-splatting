...
well-conditioned benchmark, parity is therefore structural, and an improvement there would indicate a bug, not a success.
...
Concurrent work has narrowed what can be claimed for the main idea, and the narrowing is stated here plainly, not argued away. AAA-Gaussians (2025) already
...
Several results ran against expectation, and they are reported where they happened, not collected into a footnote. Gate G2 failed, and the first
...
on. Rebuilt and
re-measured, the gate passes on all three criteria. Re-measuring then exposed a second problem, this time in the record and not in the code, in which rows that
...
\medskip
Two results are negative, and they belong here and not in a later chapter.
...
and not on the criterion; that correction is reported too. \textbf{B1 costs
