...
Several results ran against expectation, and they are reported where they happened. The scale-collapse gate failed, and the first
...
Both proposed methods are implemented and measured. The anisotropic filter reduces to its baseline
...
two decimal places, and the anisotropic filter differs from Mip-Splatting by between
...
primitives, the filter becomes anisotropic at $1.47\times$, and the anisotropic filter leads by
...
\textbf{The angular mask is refuted.} Compared against a control that has the angular mask's own
nested-degree structure and the angular mask's own budget, so that only the criterion differs,
...
freedom than the angular mask has, turned out to be mostly a verdict on the shape of the mask, leaving the
criterion itself barely implicated; that correction is reported too. \textbf{The anisotropic filter costs
