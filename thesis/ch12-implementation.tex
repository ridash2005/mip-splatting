...
says exactly what each flag does and where in the code it acts. It answers ``how do I know the 3DGS arm is really 3DGS?'' at the level of code.}
...
\begin{center}\small
\begin{tabularx}{\linewidth}{L{0.65}L{1.25}L{1.10}}
  \toprule
  ...
  the anisotropic filter & \texttt{--kernel\_size 0.1 --use\_fisher\_filter} &
    Replaces the scalar 3D filter with the anisotropic one, floored at
    Mip-Splatting's own value. \\
  the angular mask & post-hoc mask, no training flag &
    Reads a trained model and the training view directions; masks spherical-harmonic
    degrees. \\
  \bottomrule
\end{tabularx}
\end{center}
...
\paragraph{What The Compensation Switch Touches.} The flag is
threaded from \texttt{arguments/\allowbreak\_\_init\_\_.py} through
...
covariance, so replacing a scalar variance with a matrix changes how the covariance is built
and leaves the splatting untouched. Three things happen at
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item Invert the spectrum to a covariance floor, and floor \emph{that} at
        Mip-Splatting's own scalar value, taken straight from their code.
