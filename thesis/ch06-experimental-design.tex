...
\begin{table}[htbp]
  ...
  \label{tab:levels}
  \small
  \begin{tabularx}{\linewidth}{lL{0.77}L{1.62}L{0.61}}
    \toprule
...
\section{The Single-Codebase Decision}
...
\begin{figure}[htbp]
  ...
  part was the method. The published figures serve only as targets to check the reproduction against; every comparison reported in
...
implementation and one LPIPS backbone (VGG), so any difference between them is the method. The diff that separates the arms is
enumerated in the kernel and asserted on every run, and a change outside it
aborts the session before any training starts.

\keybox{what a shared codebase carries over}{%
...
Section~\ref{sec:g2} is the diagnosis and the re-measurement. The single-codebase decision stands, since it is forced. The lesson concerns the
claim made for it: ``both arms share everything but the diff'' is a statement
about what the diff \emph{omits}, and omissions are what a diff hides.}

\paragraph{The Arms, As Run.} The Mip-Splatting arm is Mip-Splatting unmodified. The 3DGS arm is the
...
\begin{table}[htbp]
  ...
  \label{tab:ladder}
  \small
  \begin{tabularx}{\linewidth}{lL{0.95}L{1.05}}
    \toprule
    ...
    R1 & Blender single-scale train $\rightarrow$ multi-scale test, 8 scenes $\times$ 2 arms & the scale-collapse gate: $\tfrac18$-scale gap exceeds 9\,dB \\
...
\begin{figure}[htbp]
  ...
  that half the images are downsampled (drawn hollow), which is a defect of camera \emph{settings}; and \emph{grazing} keeps a wide
...
with a nominal target of $6^\circ$ that comes out at $20^\circ$ is a different
capture from the one the prediction was made for, and the prediction must then be re-evaluated at the span that was built.
...
\begin{table}[htbp]
  ...
  \small
  \begin{tabularx}{\linewidth}{L{0.70}L{1.05}L{1.25}}
    \toprule
    & measurement & what it decides \\
    \midrule
    the floor-occupancy instrument & distribution of \texttt{filter\_3D} at end of training, per scene, against scene scale and distance to nearest camera & whether $s^2/\lambda_i$ ever exceeds $f_k^2$. If the floor binds almost everywhere, Proposition~\ref{prop:reduction} applies universally, the method collapses to the baseline, and the project must pivot. The cheapest possible falsification, and a by-product of a run already scheduled. \\
    the conditioning instrument & $\lambda_3/\lambda_1$ per primitive on trained, unmodified checkpoints, per capture protocol; pure PyTorch, no CUDA & whether the regime table of Equation~\eqref{eq:cap} holds on real scenes. Turns \S\ref{sec:anisotropy} from prediction into evidence before any method code is written. \\
    the repeatability measurement & per-scene mean and standard deviation across seeds, both baselines & the $\sigma$ that makes ``parity'' a quantity. Parity is then reported as within $k\sigma$ of the self-run baseline. \\
    \bottomrule
  \end{tabularx}
\end{table}

One detail here has a real failure mode. A primitive that no training camera
sees has no distance to any camera, and the published code gives it the largest distance in the scene instead of leaving it undefined. Those primitives must be
...
wrong answer at the floor-binding gate, and an encouraging-looking one.
...
floor, and the mean anisotropy of $\Sigma_{\mathrm{filt}}$; this is what makes a parity result interpretable instead of mysterious. Statistics are three seeds
...
question, so it is not done. The recovery is checked, not trusted: the
count-weighted mean of the four recovered groups must reproduce the pooled number the shipped evaluation reports, and this is asserted
on every scene of every run.
