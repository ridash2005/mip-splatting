...
kernel, which the second failure had suggested. It is not memory exhaustion:
no allocation error is raised, and \texttt{ship} completes on arm A on the same
$15\,360$\,MiB card at a measured peak of $13\,755$\,MiB. And it is not a
deterministic fault at a fixed point, which is what this section claimed before
it was instrumented --- the four attempts died at roughly iteration $3\,000$,
$10\,320$ and $12\,100$, the last two on identical code and data. What remains
is an out-of-bounds access in the forward raster whose onset depends on the
trajectory, on much the largest model in the suite: \texttt{ship} reaches
$442\,235$ primitives on arm A, more than any other Blender scene, at
$\sim\!90\,\%$ of the card.
...
\texttt{chair}, where the free control had beaten B2 by $1.82$\,dB, the fair one
beats it by $0.09$ --- inside the seed spread of $\seedSigma$\,dB, so on that
scene the criterion is not distinguishable from magnitude at all. Something like
$95\,\%$ of what looked like a verdict on identifiability was a verdict on block
structure.

What does not change is the direction. On \texttt{lego} the fair control still
wins by $0.21$ and $0.42$\,dB, two and four times the seed spread, at both
budgets where anything is pruned. So the honest statement is narrower and
