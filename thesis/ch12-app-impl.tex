\chapter{Implementation --- the two arms, and the switches between them}
\label{app:impl}
...
to ``how do I know arm B is really 3DGS?'', and it is a code-level answer.}

\section{One codebase, four configurations}
...
\begin{tabular}{llp{0.44\linewidth}}
  ...
  \midrule
  arm A & \texttt{--kernel\_size 0.1} &
    Mip-Splatting as published: 3D smoothing filter on, 2D dilation with the
    opacity compensation. \\
  arm B & \texttt{--kernel\_size 0.3 --disable\_3D\_filter
...
\section{Why arm B needs two switches, not one}
...
For a long time arm B carried only the first switch. That is the defect of
...
\section{The degeneracy guard, and why it is not part of the switch}
...
B1 needed no CUDA change. The rasteriser already accepts a precomputed
...
selected images, which is a genuinely different operation and was, for a while,
