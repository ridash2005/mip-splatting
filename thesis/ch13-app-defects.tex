\chapter{Defects found in this project's own measurement}
\label{app:defects}

\keybox{what this appendix is for, and why it is not hidden}{%
...
being wrong. They are collected here rather than scattered through
Chapter~\ref{ch:results} because a reader following the argument does not need
the forensics, and a reader auditing the work needs all of it in one place.
...
\begin{tabular}{p{0.30\linewidth}p{0.30\linewidth}p{0.30\linewidth}}
  ...
  \midrule
  Arm B reproduced 3DGS's published full-resolution figure &
  ...
  \addlinespace
  The C1 switch isolated one term &
  ...
  \addlinespace
  Table~\ref{tab:mtmt}'s arm-B column &
  ...
  \addlinespace
  B2 lost heavily to magnitude pruning &
  Against a \emph{free} control with more freedom than B2 has. At matched
  structure the margin is $\btwoBlockedGapMin$--$\btwoBlockedGapMax$\,dB &
  The control carries B2's own nested-degree structure \\
  \bottomrule
\end{tabular}
\end{center}
...
the densification explanation and B2's unfair control --- cannot be separated
from the results they correct, so they are told where those results are:
\Sef{sec:table1}, \Sef{sec:densification} and \Sef{sec:btwo}
respectively.

\paragraph{A pattern worth naming.} Six of the eight are the same error: two
...
\section{The fourth check: arm B was not running 3DGS's rasteriser}
...
\section{Every cell of this column was two rasterisers, briefly}
\label{sec:blend}
...
\section{Two kernels, two protocol definitions, one set of names}
...
\section{Getting there: three bugs the parity requirement caught}
\label{sec:b1bugs}
