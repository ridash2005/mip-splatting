...
\section{The fourth check: the 3DGS arm was not running 3DGS's rasteriser}
...
than 3DGS degrades less than 3DGS, which is exactly what the scale-collapse gate measured.

\paragraph{The opacity-compensation fix --- the test that separates the two explanations.} $\rho$ is
...
allocation error is raised in any attempt, and \texttt{ship} completes on the Mip-Splatting arm
...
from the Mip-Splatting arm's there too --- a seven-scene mean against an eight-scene one is not a
