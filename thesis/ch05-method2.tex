...
\S\ref{sec:instruments} kills the positional band-limit: the two share a principle, which is not to fit what the capture does not
determine, but they share neither a mechanism nor a failure mode. B2 also carries a memory result that B1 does
...
\paragraph{Status: implemented, measured, and refuted.} B2 is evaluated on all
...
gradually in between. What it does not do is decide well \emph{which} coefficients to keep at a fixed budget, which is the claim the
standard set below tests. Section~\ref{sec:btwo} reports the test and the criterion
fails it: against magnitude pruning restricted to B2's own nested degrees, so that the structure and the budget are
the same and only the criterion differs, B2 loses by up to
$\btwoBlockedGapMax$\,dB. The derivation that follows is therefore presented as
a proposal that was carried through to measurement and did not survive it, which
is why it is kept in full instead of trimmed to a paragraph: the
argument is sound and the conclusion is still wrong, and the gap between those
...
\S\ref{sec:btwo}, should be scheduled for a criterion that works, not for this one.