...
Think of each blob's position as a quantity you are \emph{measuring} rather than
one you are choosing. Every training photograph that sees the blob is a
measurement of where it is, and every measurement has noise --- roughly a pixel's
worth, in the image plane.

Now ask the standard estimation question: given all those measurements, how
precisely do you know the position? The answer is not one number. A camera
...
$3\times3$ matrix --- the familiar Fisher information of the measurement model,
which we write $\Lambda_k$. Its eigenvectors are the directions the capture
...
matrix, and you have a covariance floor with the right shape. Floor that at the
per-view Nyquist limit --- because no amount of accumulated information lets you
resolve structure finer than a pixel --- and you have the method.

Three things follow, and this chapter proves them. Mip-Splatting's scalar filter
is exactly what you get by collapsing this matrix to a single view and throwing
away direction. Its mysterious constant $\sqrt{0.2}$ turns out to be a confidence
radius of $0.447$ pixels. And when the Nyquist floor dominates --- which on a
normal benchmark capture it always does --- the new method becomes the old one
exactly, not approximately. That last point is why the standard benchmarks in
Chapter~\ref{ch:results} show no improvement, and why a gain there would have
indicated a bug rather than a success.}
...
\begin{figure}[htbp]
  ...
  fact that one photograph gives you direction but not distance. A second camera
  from elsewhere is precise exactly where the first is vague, and the region
...
by construction, whatever $d$ is. Off-axis, the true $J_n^{\!\top}J_n$ is not a
projector, and the two lateral eigenvalues separate.
...
\begin{figure}[htbp]
  ...
  full orbit the two ellipses coincide to within the line width, which is
  Proposition~\ref{prop:reduction} drawn rather than stated. Geometry computed
  from real pinhole Jacobians, not sketched.}
  \label{fig:fisher}
\end{figure}
...
\begin{figure}[htbp]
  ...
  one is Equation~\eqref{eq:cap} for cameras distributed over the span, which is
  what a protocol is. The points are I-2 measured on eight scenes
...
\begin{figure}[htbp]
  ...
  \caption{Why a benchmark result of ``no change'' is the correct outcome rather
  than a disappointing one. Two limits are in play and the method takes whichever
  ...
  crowd into a narrower window. On a standard benchmark --- a hundred cameras
  around a full circle --- the floor is far above the estimation limit, the
...
the implementation does exactly this. Where the filter
