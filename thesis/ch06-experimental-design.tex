...
\begin{table}[htbp]
  \centering
  \caption{The five capture protocols, as built. \emph{Span} is measured from the
  selected cameras, not specified; the prediction is Equation~\eqref{eq:cap}
  evaluated at that span.}
  \label{tab:protocols}
  \small
  \begin{tabular}{llrrp{0.3\linewidth}}
    \toprule
    protocol & selection & cameras & span & what it tests \\
    \midrule
    full orbit & all training views & 100 & $179^\circ$ & the control. Parity is required by Proposition~\ref{prop:reduction}, not hoped for. \\
    grazing & high-incidence views only & --- & $168^\circ$ & obliquity at near-full parallax; isolates the off-axis term of \S\ref{sec:offaxis}. \\
    mixed focal & all views, a seeded random half downsampled $2$--$4\times$ & 100 & $179^\circ$ & the mixed-camera defect of \S\ref{sec:defects}, not parallax. \\
    one-sided arc & contiguous azimuthal wedge & 25 & $80^\circ$ & the anisotropic regime: the floor binds in some directions and not others. \\
    low-parallax cone & tightest angular cluster & 10 & $20^\circ$ & the largest predicted gain, and the only protocol where the floor is exceeded in every direction. \\
    \bottomrule
  \end{tabular}
\end{table}
