...
across a \emph{distribution} of views spread over some angular window, which is
what a real capture protocol is, the correct expression is a spherical-cap form
...
as much as $\twoViewLowMax\,\%$. The third result is a guarantee rather than a
formula: if the anisotropic floor is itself floored at Mip-Splatting's own
value, then wherever that value dominates the two methods coincide exactly. On a
well-conditioned benchmark, parity is therefore structural, and an improvement
there would indicate a bug rather than a success.
...
Concurrent work has narrowed what can be claimed for the main idea, and the
narrowing is stated here rather than argued away. AAA-Gaussians (2025) already
makes the 3D filter a full $3\times3$, though derived from the view being
rendered rather than from the capture. PUP 3D-GS (2025) already builds a
...
anisotropy nor the matrix, but where the anisotropy comes from --- the
conditioning of the training capture, fixed once per primitive --- and the
decision to keep its whole spectrum rather than a summary of it.
...
Several results ran against expectation, and they are reported where they
happened rather than collected into a footnote. Gate G2 failed, and the first
diagnosis of that failure was itself wrong: the arm labelled 3DGS had kept
Mip-Splatting's screen-space opacity compensation, so it was anti-aliasing
better than 3DGS at exactly the scales the gate was scored on. Rebuilt and
re-measured, the gate passes on all three criteria. Re-measuring then exposed a
second problem, this time in the record rather than the code, in which rows that
...
The step falls exactly where the estimation term overtakes the floor, which is
what was predicted and recorded beforehand.
...
\medskip
Two results are negative, and they belong here rather than in a later chapter.
...
freedom than B2 has, turned out to be mostly a verdict on the shape of the mask
rather than on the criterion; that correction is reported too. \textbf{B1 costs
