...
\usepackage{array}
\usepackage{tabularx}
% Proportional text columns. tabularx solves for the widths so that they sum
% to the given total, so a table declared this way cannot overflow the
% margin whatever lands in a cell. The argument is the column's share; the
% shares in one table must add up to the number of such columns.
...
% Last-resort stretch. A paragraph carrying an unbreakable file path or
% command-line flag comes out slightly loose instead of past the margin;
% TeX reaches for this only after the ordinary passes have failed.
...
\fancyhead[L]{\footnotesize\color{ink2} Sampling Geometries and Band-Limits for Radiance Fields}
