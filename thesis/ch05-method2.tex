...
gradually in between. What it does not do is decide well \emph{which}
coefficients to keep at a fixed budget, and that is the claim the standard set
below actually tests. Section~\ref{sec:btwo} reports the test and the criterion
fails it: against magnitude pruning restricted to B2's own nested degrees ---
same structure, same budget, only the criterion differing --- B2 loses by up to
...
is the reason it is kept in full rather than trimmed to a paragraph: the
...
\S\ref{sec:btwo}, should be scheduled for a criterion that works rather than for
this one.