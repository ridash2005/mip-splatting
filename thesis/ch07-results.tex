...
all? Nothing later means anything if not. Then the two instruments, which measure
the capture geometry itself rather than any rendered image --- these are the
...
and a quantity that has not been measured appears as a visible marker rather than
as a plausible figure. Published figures are cited as \emph{targets} and never
...
\section{Before any measurement: the pre-flight}
\label{sec:preflight}

A reproduction that has not checked its own instrument is a reproduction of
nothing in particular. Twelve assertions run before the first training step ---
that the GPU is one the code can run on, that both arms load the same images,
that the metric uses the backbone the papers used, that the two checkouts differ
by exactly the intended diff and nothing else --- and the session aborts if any
fails. They are listed, with what each protects against, in
Appendix~\ref{app:harness}. All twelve pass.
...
\begin{figure}[htbp]
  ...
  remaining scenes are kept as per-scene contact sheets alongside the record, so
  that the selection can be audited rather than trusted.}
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
  \item \textbf{The per-scale recovery.} Its count-weighted mean reproduces
        ...
        scene of every run --- asserted per scene, not spot-checked. Not the
        cause.
...
Arm B was built by zeroing Mip-Splatting's 3D smoothing filter, which makes the
world-space band-limit 3DGS's. But Mip-Splatting has a \emph{second}
band-limit --- a screen-space dilation with an opacity compensation
$\rho = \sqrt{\det\Sigma' / \det(\Sigma' + k I)}$ --- and that one lives in CUDA,
where zeroing a Python attribute does not reach it. The arm labelled 3DGS was
therefore anti-aliasing better than 3DGS, at exactly the scales where the gate
was scored.

Isolating that term is change C1. Appendix~\ref{app:impl} gives the switch and
...
Removing the compensation cost something the protocol had not budgeted for.
\texttt{ship} --- much the largest scene in the suite at $442\,235$ primitives
and $90\,\%$ of the card --- does not train on the vanilla arm at all. It has
...
Two things make the exclusion narrow rather than convenient. \texttt{ship} trains
...
And it trains without incident on arm A, so the fragility belongs to the vanilla
band-limit semantics rather than to the scene, the data or the harness.
...
\paragraph{This column was two rasterisers, briefly.} Arm B was re-measured
after C1, and for a period each of its $32$ cells was the mean of the old
measurement and the new one rather than the new one --- the rows the re-run
...
$45$ lines across two files and nothing else; it is the configuration
Mip-Splatting's published table was produced at. Arm A was re-run on it, all
...
\paragraph{Three bugs on the way here.} Requiring parity \emph{exactly}
rather than approximately turned the reduction theorem into a test, and the test
...
\section{What it cost to run}
\label{sec:budget}

Every number in this chapter was produced on free cloud GPUs, within a weekly
allowance that the project exhausted more than once. The ladder was designed
around that constraint rather than in spite of it, and the accounting --- which
rung cost what, which session died and why, and what the constraint forbade
outright --- is set out in Appendix~\ref{app:harness}. The short version is that
compute, not code, was the binding constraint throughout.