...
Think of each blob's position as a quantity you are \emph{measuring}, not one you are choosing. Every training photograph that sees the blob is a
...
always does, the new method becomes the old one exactly, not approximately. That last point is why the standard benchmarks in
Chapter~\ref{ch:results} show no improvement, and why a gain there would have indicated a bug, not a success.}
...
It is not asserted as a bound this work verifies. What is asserted, and what the experiments test, is
...
implementation uses the full $J_nW_n$ and never the rank-1 shortcut, so this term is retained and never recovered; the shortcut appears in this document
...
Notice that these are three distinct numbers, not two. The right filter is therefore triaxial, not merely elongated along depth: the lateral
...
\begin{figure}[htbp]
  ...
  full orbit the two ellipses coincide to within the line width, which is Proposition~\ref{prop:reduction} drawn instead of stated. Geometry computed
...
\begin{figure}[htbp]
  ...
  one is Equation~\eqref{eq:cap} for cameras distributed over the span, as a protocol is. The points are I-2 measured on eight scenes
...
This proposition is implemented as a unit test, not left as prose: with
...
\S\ref{sec:onecodebase}, and Chapter~\ref{ch:results} reports it verified on the saved models, not asserted.

\section{What an anisotropic filter costs the implementation}
\label{sec:implcost}

Two consequences follow from $\Sigma_{\mathrm{filt}}$ being a matrix and not a scalar. Both are properties of the code, not of the
derivation.
...
and passed through the rasteriser's precomputed-covariance input. The implementation does exactly that. Where the filter comes out isotropic, which Proposition~\ref{prop:reduction}
makes the common case, it takes Mip-Splatting's own scalar path instead, so parity is
...
degenerate eigenvector basis does not arise, though it is where a naive implementation of
