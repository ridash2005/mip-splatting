...
That is inside the $\pm0.20$\,dB of Table~\ref{tab:levels}, so arm A carries an
...
$\stmtArmBScenes$-scene mean standing against an eight-scene published one. Its
$\stmtArmBMaxDelta$\,dB is not a reproduction error and is not reported as one;
it is mostly the missing scene.}
...
against published figures that are means over eight. \texttt{ship} is arm A's
weakest scene at full resolution --- $30.48$\,dB against a set mean of
$33.44$ --- so excluding it lifted arm A's mean by $0.42$\,dB, and that entire
lift was being reported as reproduction error. Arm A read $0.50$\,dB from its
...
\begin{figure}[htbp]
  ...
  numbers are PSNR for the two images shown, recomputed from those pixels, not
  scene means --- which is why they differ from Table~\ref{tab:stmt}, a mean
...
\begin{figure}[htbp]
  ...
  these four are shown because they are visually distinct from one another, and
  the remainder are in \texttt{results/qualitative/} as per-scene contact
  sheets so the selection can be audited.}
...
survives is a matched set of predictions and ground truths per model; these are
those files, not a re-render made for the figure. The ground-truth image is
byte-identical between the two arms at every index, checked rather than assumed,
...
to within $\stmtGapMaxDelta$\,dB at all four, which is inside the L1 tolerance. This project did not have that before; it had arm A at L1 and an arm
...
Three things that rules out, and one it does not. It is not the backward
kernel, which the second failure had suggested. It is not memory exhaustion: no
allocation error is raised in any attempt, and \texttt{ship} completes on arm A
on the same $15\,360$\,MiB card at a measured peak of $13\,755$\,MiB. And it is
not a deterministic fault at a fixed point, which is what this section claimed
before it was instrumented.
...
whose onset moves is a fresh draw each time and retrying it is measurement
rather than hope. \textbf{All nine failed}, every one with
...
is reported as failed rather than re-attempted further.
...
\texttt{ship} reaches $442\,235$ primitives on arm A, more than any other
Blender scene, at $\sim\!90\,\%$ of the card.

It is excluded from the single-scale arm-B rows, and because every between-arm
table in this chapter is restricted to the scenes both arms have, it is excluded
from arm A's there too --- a seven-scene mean against an eight-scene one is not a
comparison. Nothing was reduced to make it fit: the protocol's rule for a scene
that will not run is to record it and move on, and that rule does not become
optional when the scene is one of eight rather than one of nine.

Two things make the exclusion narrow rather than convenient. \texttt{ship}
...
specific to single-scale training. And it trains without incident on arm A, so
the fragility belongs to the vanilla band-limit semantics rather than to the
scene, the data or the harness. Both point the same way: removing the
...
\subsection*{Every cell of this column was two rasterisers, briefly}
\label{sec:blend}

Arm B's column here was re-measured after C1, and for a while afterwards each of
its $32$ cells was the mean of the old measurement and the new one rather than
the new one. The record is append-only by design (Table~\ref{tab:levels}): a
...
taken hold in the B1 parity rows, where the two builds differed in whether the
...
\keybox{append-only is a property of the file, not of the average}{%
...
What it does \emph{not} explain is arm A's distance from its published figure,
and for two independent reasons. The first is that there is no longer much
distance to explain: on the eight scenes the published mean covers, arm A is
...
Commit \texttt{1ecf7d5} is the last before ``add GOF densification'' landed. It
differs from the checkout every other table in this chapter was measured on by
$43$ lines in \texttt{scene/gaussian\_model.py} and $2$ in \texttt{train.py},
and nothing else; it is the configuration Mip-Splatting's published table was
produced at. Arm A was re-run on it, all $\pregofScenes$ scenes, same data, same
seed, same iterations.

\keybox{the densification explanation is refuted}{%
At the pre-GOF commit arm A reproduces the published figures to
...
So the densification difference is real, it is shared by both arms, and it is
\emph{not} why arm A sits where it sits. It was never tested until now, and it
was wrong. The sentence it replaces --- ``a better densification plausibly
accounts for standing above the published level'' --- was doing the work of a
measurement while being an assertion, and the word \emph{plausibly} was carrying
all of it.}

The earlier version of this section also argued the point from arm B's
...
\begin{enumerate}[leftmargin=1.4em,itemsep=3pt]
  \item \textbf{Arm A carries an L1 claim on PSNR; arm B carries none.} Arm A is
        within $\stmtArmAMaxDelta$\,dB over the eight scenes the published mean
        covers. Arm B is missing \texttt{ship}, so its column is a
...
is still true. What has changed is that the cost of keeping GOF is now measured
rather than assumed, and it is $\gofMaxEffect$\,dB at worst.
...
mean has a standard error of $\seedSigma/\sqrt{3}$ and is comfortable; a single
run is adequate rather than generous. Both arms' reproductions in
\S\ref{sec:table1} are single-seed results, and several of their margins are of
the same order as this $\sigma$. They should be read as L1 attained, not as L1
attained with room to spare.
...
\subsection*{Two kernels, two protocol definitions, one set of names}
...
names, and it is worth saying exactly how, because nothing in either run could
have detected it.
...
$20^\circ$ wedge catches three cameras on a Blender capture, and three views is a
sparse-reconstruction experiment rather than a conditioning one. What was missed
...
\keybox{what it cost, and what it bought}{%
It cost the first reading of gates G3 and G4 --- and it bought the \texttt{pencil}
...
floor, and with both in the table that crossover is a measurement rather than an
extrapolation. The instrument kernel is now pinned to the branch the training
kernel clones, and every protocol table carries the span the session that
produced the row actually built.}

\subsection*{A constant that was carried on one side only}
\label{sec:m2defect}
...
built from. Both sides of the I-1 comparison therefore carry it. The instrument
...
\keybox{Proposition~1 is a test, not only a statement}{%
...
cannot both be free --- surfacing as a live numerical error rather than as an
ambiguity in the prose, which is the more useful way for it to surface.}

\subsection*{I-1: where the floor binds, and where it stops}
...
On the standard benchmark the floor dominates by a factor of six, on
\textbf{zero} primitives in \emph{any} direction of \emph{every} scene. By
Proposition~\ref{prop:reduction} that makes B1 \emph{be} Mip-Splatting there ---
exactly, not approximately --- and it is worth stating plainly:
...
I-1 is computed off a trained point cloud by a NumPy instrument; the filter
...
They agree on five of six protocols: $0\,\%$ on the full orbit, grazing, mixed
focal and the arc, and the pencil above the floor on essentially everything
($100\,\%$ against the filter's $93\,\%$). The one disagreement is the cone,
where the instrument reads $100\,\%$ and the filter $0\,\%$ --- and the ratio
there is $1.35$, within $35\,\%$ of unity, which is precisely where a finely
balanced verdict should be sensitive to what it is computed on. The instrument
reads a model trained on the \emph{full orbit} and asks what the cone's cameras
would have determined about it; the filter reads the model the cone's cameras
actually produced, which has different primitives at different depths and
therefore a different floor. Both are correct about their own question, and the
operative one --- the one that decides what the filter does --- is the filter's.
...
Every measured anisotropy exceeds the two-view prediction of
Equation~\eqref{eq:anisotropy}, and by roughly the same factor throughout. That
is not five independent discrepancies; it is one, and \S\ref{sec:capprediction}
names it. A protocol is a distribution of cameras over an angular window, not two
...
evaluated at each protocol's own measured span, the $\capAgreeN$ protocols the
cap model is built for agree to $\capAgreeMin$--$\capAgreeMax\,\%$, where the
two-view form falls short by up to $\twoViewLowMax\,\%$. The sixth,
\emph{grazing}, sits at $\capAgreeGrazing\,\%$, and is precisely the protocol
the cap model does not describe: it was built to test obliquity at near-full
parallax, and the cap form is parallax-only. The residual is one-signed ---
measurement always above prediction --- which is what \S\ref{sec:offaxis} leads
one to expect: the cap model is parallax-only, and perspective adds anisotropy of
its own that a cap of ray directions does not contain.}
...
$+0.017$\,dB. The honest reading is that the mixed-camera defect is a statement
about the \emph{value} of $f_k$ and not about its isotropy, and that
...
B1 densifies to within $0.2\,\%$ of Mip-Splatting's primitive count, which is
...
and rendering is unaffected --- the filter is applied once at construction, not
per frame (\S\ref{sec:fps}).

Training is another matter. B1 costs $2.8\times$ the peak VRAM and $1.32\times$
...
the peak does not track the primitive count --- \texttt{ficus} at $187$k
primitives and \texttt{mic} at $406$k both peak near $13$\,GB --- which is what
a large transient in that accumulation looks like rather than a persistent
allocation.

That is the honest cost line, and it is not a small one. A practitioner reading
this table on a well-conditioned capture should not use B1: it is the same
...
and raced on the point cloud the loader writes into the source directory --- B1
...
It is not re-run, and the reason is this table rather than the schedule.
Mip-Splatting alone peaks at $14\,459$\,MB on \texttt{ship}, against a
$15\,360$\,MB card. At the $2.8\times$ measured here, B1 on \texttt{ship} does
...
Proposition~\ref{prop:reduction} says B1 \emph{becomes} Mip-Splatting when the
...
Mip-Splatting at the full protocol, so B1 was being differenced against a sweep
...
now enforced in the generator rather than assumed, and why the scene count is a
macro rather than the word ``eight''.}
...
$\stressBindDelta$\,dB on the mean --- $\stressBindSigma\times$ the run-to-run
spread --- and, more to
...
\begin{figure}[htbp]
  ...
  \caption{The operating point where B1 acts, rendered. Three training cameras
  spanning $29^\circ$; the test set is the full orbit, so most of what is
  evaluated was never observed. Numbers are PSNR for the images shown. B1 leads
...
and nothing in either image would be accepted as output. B1's advantage is real,
...
this operating point. A capture this degraded needs more cameras, not a better
band-limit.}
...
the same reason B1 had to be, across all six capture protocols.
...
machinery'' --- and by that standard the criterion fails, on the fair comparison
...
asymmetry in the free control, and it is not small: B2 masks whole
...
against it is evidence about the \emph{structure} of the mask and not about the
criterion --- and the criterion is what is on trial. The first version of this
section noted the confound and left it, which was the right thing to notice and
the wrong place to stop.

The control that separates them is magnitude pruning restricted to B2's own
...
\btwoStructureScene, where the free control had beaten B2 by
...
rather than by two decibels, on one scene of two, and not by enough on the other
to call.
...
About the proposed method they establish four things and refute one. B1 is
...
test scales in paired training runs. The conditioning both methods depend on
varies across capture protocols exactly as Equation~\eqref{eq:cap} predicts.
...
isotropic to two decimals, and on the one where the estimation term binds B1
leads by $\stressBindDelta$\,dB, on the mean and on every paired cell. And the
cost of that is now measured rather than assumed: $2.8\times$ peak VRAM and
$1.32\times$ training time for a model with the same primitive count
(\S\ref{sec:methodcost}).

What is refuted is B2. At matched structure and matched budget it loses to
...
What is \emph{not} established is that B1 is useful. The regime where it helps
